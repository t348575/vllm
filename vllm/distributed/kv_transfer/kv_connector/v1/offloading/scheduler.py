...
class OffloadingConnectorScheduler:
    ...
    def __init__(self, spec: OffloadingSpec):
        ...
        # we eagerly build a load spec for the hit blocks so the worker storage
        # handler can start reading into staging before the GPU blocks are
        # allocated. _reqs_to_preload is drained into the per-step metadata;
        # _req_preload_specs caches the spec per request for reuse by
        # update_state_after_alloc (keyed by the exact hit keys).
    ...
    def _maybe_queue_preload(
        ...
        if existing is None or existing[0] != hit_keys:
            src_spec = self.manager.prepare_load(hit_keys, req_status.req_context)
            offloaded_block_size = self.config.kv_group_configs[0].offloaded_block_size
            start_block_idx = (
    ...
    def update_state_after_alloc(
        ...
        if self.enable_preload:
            # Reuse the spec built at lookup time when the hit keys match, so
            # the preload_id flows onto the real load job and the worker can
            # place the already-preloaded blocks instead of re-reading.
            preload = self._req_preload_specs.get(request.request_id)
            if preload is not None and preload[0] == tuple(keys_to_load):
                src_spec = preload[1]
    ...
    def update_connector_output(self, connector_output: KVConnectorOutput):
        ...
        for job_id, count in meta.completed_jobs.items():
            ...
            if not req_status.transfer_jobs and req_status.req.is_finished():
                ...
                self._clear_preload_state(job_status.req_id)
    ...
    def request_finished(
        ...
        self._clear_preload_state(request.request_id)
