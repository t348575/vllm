...
class OffloadingConnectorMetadata(KVConnectorMetadata):
    ...
    # Scheduler -> worker preload hints (req_id -> load spec carrying a
    # preload_id attribute). Lets storage backends begin reading hit blocks
    # early; the matching load job places the already-read data.
    reqs_to_preload: dict[ReqId, LoadStoreSpec] | None = None


@dataclass
class OffloadingWorkerMessageMetadata(KVConnectorMetadata):
    """Scheduler -> worker arbitrary message relay (e.g. preload kickoff)."""
